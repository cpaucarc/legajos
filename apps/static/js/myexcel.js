        ...
        excel.set = function (s, column, row, value, style, colspan) {
            ...
            if (!isNaN(column)) {                                                                                                // If this is a column operation
                if (!isNaN(row)) {
                    ...
                    return setCell(s.getCell(column, row), value, style, true, colspan);                                                // and also a ROW operation the this is a CELL operation
                }
...
function exportarExcel(result, nombre){
    var excel = $JExcel.new("Arial 10 #000000");

    for (var i=0;i<result.cabecera.length;i++){
        excel.set(0,i,0,result.cabecera[i]);
    ...
    for (var i=0;i<result.data.length;i++){
        ...
        for(var y=0;y<item.length;y++){
            excel.set(0,y,filaExcel, item[y]);
            excel.set(0,y,undefined,"auto");
        }
        filaExcel = filaExcel + 1;
    }
    var f = new Date();
    var cero1 = cero(f.getDate());
    var cero2 = cero(f.getMonth() + 1);
    var cero3 = cero(f.getHours());
    var cero4 = cero(f.getMinutes());
    var cero5 = cero(f.getSeconds());
    excel.generate(nombre+"_"+f.getFullYear()+""+ cero2 + (f.getMonth() +1)+""+ cero1 + f.getDate()+"_"+ cero3 + f.getHours()+""+ cero4 + f.getMinutes()+""+ cero5 + f.getSeconds()+".xlsx");
