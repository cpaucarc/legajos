...
table_archivos.on("draw", function(){
  $('#tabla-archivos td').css('white-space','initial');
...
$(document).on('click', '.archivose', function(e) {
  $("#cargando-archivos").hide();
  var id = $(this).attr('data-id');
  var tipo = $(this).attr('data-tipo');
  $('#modal-archivos').modal('show');
  $("#myDropzone").attr('action', urlSubirArchivos.replace('id', id).replace('tipo', tipo));
  table_archivos.ajax.url(urlListaArchivos.replace('id', id).replace('tipo', tipo)).load();
});
