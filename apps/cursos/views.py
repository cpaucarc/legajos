...
def cursos_create_view(request, pk):
    ...
    if request.method == "POST":
        ...
        mensaje = "¡La información de los {} cursos fue registrado con éxito!".format(cant_registrados) if cant_registrados > 0 else None

    form = CursosCrearForm(None)
    cursos_registrados = CursoDictado.objects.filter(persona_id = pk).order_by("institucion_id", "semestre_id", "escuela", "curso")
